...
const faces = pickFaces(library, people.map((p) => p.toUpperCase()), expression ?? null, kind === "reaction" ? 2 : 4);
...
console.log("ok", out, faces.map((f) => `${f.label}:${f.expression}:${f.intensity}`).join(" "));
