  ...
  return (
    <li className="space-y-2 rounded-lg border border-line bg-surface p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="min-w-0 break-words text-sm text-ink">{job.clipTitle}</span>
      ...
      {job.status === "COMPLETED" && (
        <>
          {previewUrl ? (
            <img src={previewUrl} alt={job.clipTitle} className="w-full max-w-md rounded-lg border border-line" />
          ) : (
            <button
              onClick={loadPreview}
              disabled={loading}
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium ${color[status] ?? "bg-zinc-700 text-ink"}`}>
      {label[status] ?? status}
    </span>
  );
}
